...
document.addEventListener('DOMContentLoaded', () => {
  const estadoSelect = document.getElementById('estado-servicio');
  const ciudadSelect = document.getElementById('ciudad-servicio');
  ...
  const serviciosPorSucursal = {
    "Cumbres": ["Servicio Funerario Básico"],
    "San Bernabé": ["Servicio Funerario Básico"],
    "Apodaca": ["Servicio Funerario Básico"],
    "Guadalupe": ["Servicio Funerario Básico"],
    "Escobedo": ["Servicio Funerario Básico"],
    "Saltillo": ["Servicio Funerario Básico"],
    "Torreón": ["Servicio Funerario Básico"]
  };

  const servicioImagenes = {
    "Servicio Funerario Básico": "../1.6imagenes/collage-servicio-funerario-basico-1.jpg"
  };

  const rutasRedireccion = {
    "Servicio Funerario Básico": "../1.1servicios/1.1.2.1servicio-funerario-basico.html"
  };
  ...
  ciudadSelect.addEventListener('change', () => {
    ...
    if (!sucursal || !serviciosPorSucursal[sucursal]) return;

    const servicios = serviciosPorSucursal[sucursal];

    servicios.forEach((nombre, i) => {
      const div = document.createElement('div');
      div.classList.add('propiedad-funeraria', 'aparecer');
      div.innerHTML = `
        <img src="${servicioImagenes[nombre]}" alt="${nombre}">
        <h4>${nombre}</h4>
      `;
...
// Asegúrate que exista la animación CSS con .aparecer y @keyframes gelatina
